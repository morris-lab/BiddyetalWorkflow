...
We use the \*.possorted\_genome\_bam.bam output file from the 10x CellRanger pipeline, because each read has been tagged with its associated Cell Barcode, UMI, and aligned genes.
...
1.  The input file `v1.celltag.reads.out`.
...
Now that the reads have been parsed we can quantify the CellTag for each cell and create a Cell x CellTag matrix. We accomplish this using a custom R script that can be found in the `/scripts/` directory.
